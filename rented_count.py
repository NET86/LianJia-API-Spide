        ...
        while True:
            if self.queue.empty():  # 如果队列为空，则跳出
                ...
                if total_count <= 2000 and total_count != 0:
                    ...
                    # 如果大于2000条，调用do_rented_2000分段
                    rented_split = do_rented_2000(bizcircle['city_id'], bizcircle['condition'])
                    for rs in rented_split:
                        # 此处有大坑，bizcircle_tmp应在此处赋值，如在之前赋值会有问题
                        bizcircle_tmp = bizcircle.copy()  # 复制一个对象出来，用COPY，不能用=
                        brp = list(rs.keys())[0]
                        erp = list(rs.values())[0]
                        bizcircle_tmp['condition'] = bizcircle['condition'] + 'brp{}erp{}'.format(brp, erp)
